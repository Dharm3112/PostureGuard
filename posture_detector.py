...
class PostureDetector:
    ...
    def __init__(self, buffer_size: int = 10, scale_factor: float = 1.1, min_neighbors: int = 5) -> None:
        ...
        self.scale_factor = scale_factor  # Set resolution sizing scale scaling parameters values filter
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Optional[float]]:
        ...
        # Detect target faces coordinates using Haar patterns searches
        ...
        if len(faces) > 0:
            self.consecutive_failures = 0  # Reset consecutive failure counters check
