...
class PostureApp:
    ...
    def calibrate(self) -> None:
        ...
        if baseline:
            ...
            self.frames_bad = 0  # Reset bad posture frame counters on recalibration
    ...
    def check_posture(self, current_y: Optional[float]) -> None:
        ...
        # Check if vertical pixel offset exceeds standard calibration range threshold
        ...
        # Alerting Logic
        # Check if slouch tracking duration exceeds configured limit settings constraints
        if self.frames_bad > self.TIME_TO_ALERT:
            ...
            if self.frames_bad % 100 == 0:  # Sound alert occasionally to avoid spam
                # Log slouching duration and notification dispatch event
